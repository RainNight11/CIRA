...
def encode_images_visionzip(self, images):
    image_features, _ = self.get_model().get_vision_tower().forward(images)
...
def encode_images_visionzip_multi(self, images):
    ...
    return self.get_model().mm_projector(image_features), keep_idx
...
def restore_image_features_sorted(self, image_feature, cur_keep_idx, width, height):
    ...
    assert width * height == restored_features.shape[0]
    restored_features = restored_features.view(height, width, 24, 24, feature_dim)
    restored_features = restored_features.permute(0, 2, 1, 3, 4).contiguous()
    restored_features = restored_features.view(height * 24, width * 24, feature_dim)
    grid_with_newline = restored_features

    mask = mask.view(height, width, 24, 24)
    mask = mask.permute(0, 2, 1, 3).contiguous()
    mask = mask.view(height * 24, width * 24)

    image_feature_select = grid_with_newline[mask]
    if num_extra:
        raw_img_feature_merge = image_feature[:, -num_extra:, :].reshape(
            -1, feature_dim
        )
    else:
        raw_img_feature_merge = image_feature.new_empty((0, feature_dim))
    cls_img_feature_merge = image_feature[:, 0, :]

    image_feature_select = torch.cat(
        [image_feature_select, cls_img_feature_merge, raw_img_feature_merge]
    )
